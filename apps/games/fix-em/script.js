...
function showPassage(){isChecking=false;const p=playPassages[currentStep];document.getElementById('progress-text').textContent=`${currentStep+1}/${playPassages.length}`;document.getElementById('score-text').textContent=score;document.getElementById('feedback-controls').classList.add('hidden');document.getElementById('main-check-btn').classList.remove('hidden');hideExplanation();document.getElementById('passage-tag').textContent=`#${currentStep+1} · ${p.level}`;document.getElementById('passage-title').textContent=p.title;updateDots();const passageText=document.getElementById('passage-text');passageText.innerHTML=escapeHtml(p.text);fitText(passageText,document.getElementById('passage-fit-container'),32,14);}

function revealAnswers(){if(isChecking)return;isChecking=true;const p=playPassages[currentStep];const errors=p.errors.filter(e=>e.errorText.trim()&&e.correction.trim());const passageEl=document.getElementById('passage-text');let html=escapeHtml(p.text);errors.forEach(e=>{const re=new RegExp(escapeHtml(e.errorText),'i');html=html.replace(re,match=>`<span class="passage-word marked-missed">${match}</span>`);});const currentSize=passageEl.style.fontSize;passageEl.innerHTML=html;if(currentSize)passageEl.style.fontSize=currentSize;const explanationContent=document.getElementById('explanation-content');const explanationHTML=errors.map(e=>`<div class="mb-2 last:mb-0"><div class="flex items-center gap-2 mb-0.5"><i data-lucide="alert-circle" class="w-3.5 h-3.5 text-orange"></i><span class="text-xs font-black text-orange uppercase tracking-widest">ERROR</span></div><p class="text-sm font-bold text-dark dark:text-white">"${escapeHtml(e.errorText)}" → "${escapeHtml(e.correction)}"</p>${e.explanation?`<p class="text-xs text-slate-500 dark:text-slate-400 font-semibold mt-0.5">${escapeHtml(e.explanation)}</p>`:''}</div>`).join('');explanationContent.innerHTML=explanationHTML;showExplanation();score+=errors.length;document.getElementById('score-text').textContent=score;document.getElementById('feedback-controls').classList.remove('hidden');document.getElementById('main-check-btn').classList.add('hidden');AudioEngine.correct();confetti({particleCount:80,spread:70,origin:{y:0.7},colors:['#00E676','#FF8C42','#2979FF']});lucide.createIcons();}
...
window.addEventListener('keydown',(e)=>{if(isSetupMode)return;const gameScreen=document.getElementById('game-screen');if(gameScreen.classList.contains('hidden'))return;if(!isChecking){if(e.key.toLowerCase()==='r'){e.preventDefault();revealAnswers();return;}}if((e.key==='Enter'||e.key===' ')&&!document.getElementById('feedback-controls').classList.contains('hidden')){e.preventDefault();nextPassage();return;}});
